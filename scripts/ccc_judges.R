...
data_judges <- tribble(
  ...
  "Pavel Holländer", "Pav[a-zěščřžýéáó]*\\s*Holländer[a-zěščřžýéáó]*", 1953, "M", "KOM", "prof", "scholar", c("15.7.1993", "6.8.2003"), c("15.7.2003", "6.8.2013"),
  "Vojen Güttler", "Vojen[a-zěščřžýéáó]*\\s*G(ü|ű|u|ú)t(|t)ler[a-zěščřžýéáó]*", 1934, "M", "CUNI", "judr", c("politician", "judge"), c("15.7.1993", "6.8.2003"), c("15.7.2003", "6.8.2013"),
  ...
  mutate(judge_term_court = case_when(year(judge_term_start) < 1995 ~ "1st",
         ...
         reelection = if_else(judge_name %in% c("Pavel Rychetský", "Jiří Nykodým", "Ivana Janů", "Jan Musil", "Pavel Holländer", "Vojen Güttler", "Miloslav Výborný"), true = 1, false = 0)) %>%
  relocate(judge_id) %>%
